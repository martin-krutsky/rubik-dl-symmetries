...
if __name__ == "__main__":
    ...
    compressions, hash_to_sizes = calculate_all_dicts_from_data(df=df, max_distance=N_MOVES,
                                                                input_handling_func=heuristic_function)

    with open(f'data/temp/{heuristic_dict[HEURISTIC]}_compressions_distance{N_MOVES}.pkl', 'wb') as f:
        pickle.dump(compressions, f)

    with open(f'data/temp/{heuristic_dict[HEURISTIC]}_hashtosizes_distance{N_MOVES}.pkl', 'wb') as f:
