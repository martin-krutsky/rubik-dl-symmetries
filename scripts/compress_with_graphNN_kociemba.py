...
gcn_compressions = calculate_all_dicts_from_activations(df=df, max_distance=None, input_handling_func=create_complete_graph_data_obj, networks=networksCompGraph, is_graph_nn=True)

with open(f'data/temp/symeqnet_compressions_kociemba.pkl', 'wb') as f:
    pickle.dump(gcn_compressions, f)
    
set_intersections_activations_sparse_graph = compute_set_intersections(gcn_compressions, distances=df['distance'].unique())
plot_distance_compressions(gcn_compressions, f'symeqnet_compressions/from_activations_kociemba', distances=df['distance'].unique())
